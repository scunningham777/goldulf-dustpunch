...
export class Token extends Phaser.GameObjects.Image {
    ...
    /**
     * @param onComplete Optional callback invoked after the token has finished its
     * move-to-inventory animation (just before it is destroyed).
     */
    constructor(
        scene: Phaser.Scene,
        x: number,
    ...
    animateToInventory(onComplete?: () => void) {
        ...
        this.scene.tweens.add({
            ...
            duration: 1000,
            onComplete: () => {
                // then fly to the corner (inventory)
